    ...
    def search_cache(self, query: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                return {
                    "hit": len(response.json()) > 0,
                    "results": response.json(),
                    "latency_ms": response.elapsed.total_seconds() * 1000
                }
            else:
                logger.warning(f"Cache search failed: {response.status_code}")
...
def shadow_llm_call(llm_function: Callable, query: str, *args, **kwargs) -> Any:
    ...
    # Extract response text (this may need customization based on LLM provider)
    ...
    if config.enabled and langcache_client and shadow_logger:
        _perform_shadow_operations(query, response_text, llm_latency)
...
def _perform_shadow_operations(query: str, llm_response: str, llm_latency: float):
    ...
    def _shadow_ops():
        try:
            ...
            shadow_data = {
                ...
                "cache_response": cache_result["results"][0].get("response") if cache_result["results"] else None,
                "similarity_score": 1.0 - cache_result["results"][0].get("distance", 1.0) if cache_result["results"] else None
            }
